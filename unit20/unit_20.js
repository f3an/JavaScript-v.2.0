...
function t5(event) {
  ...
  event.which >= 65 && event.which <= 90 && event.shiftKey == true
    ? (out.textContent += event.key)
    : (out.textContent += "");
}
...
function t7() {
  ...
  const i = Math.floor(Math.random() * a7.length);
  out.textContent += `${a7[i]}, `;
}
...
function t9(event) {
  ...
  event.key === "ArrowDown" ? counter++ : counter;
...
function t11(event) {
  ...
  for (const i of keyboard) {
    if (i.getAttribute("data") == event.key) {
      i.classList.add("active");
    } else if (event.keyCode == 32) {
      i.getAttribute("data") == "space" ? i.classList.add("active") : i;
    } else if (event.keyCode == 17) {
      i.getAttribute("data") == "ctrl" ? i.classList.add("active") : i;
    }
  }
}
