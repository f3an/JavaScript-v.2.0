...
// Вывод в out-6
const MyBirthDay = new Date("02-06-2000");

let ar6 = [
  "Ihro",
  "Aquaris",
  MyBirthDay.getDate(),
  MyBirthDay.toLocaleString("default", { month: "long" }),
];
...
function f8(out1, out2, elem) {
  out1 = document.querySelector(".out-8");
  out2 = document.querySelector(".out-8-1");
  ar8[3] = 3.14;
  ar8[4] = 17;
  ar8[6] = 5;
  elem = "";
  ...
  out1.innerHTML = elem.substring(0, elem.length - 1);
  out2.innerHTML = ar8.length;
}
...
function f16(out_odd,out_even) {
  out_odd = document.querySelector(".out-16-odd");
  out_even = document.querySelector(".out-16-even");
  for (let i = 1; i < ar16.length; i++) {
    (!(ar16[i]%2))?
    ar16_even.push(ar16[i]):
    ar16_odd.push(ar16[i]);
  }
  out_even.innerHTML = ar16_even;
  out_odd.innerHTML = ar16_odd;
  ar16_odd = [];
  ar16_even = [];
}
...
function f19(out,temp) {
  out = document.querySelector(".out-19");
  temp = Math.min.apply(Math, ar19);
