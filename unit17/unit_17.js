...
function t3() {
  a3_res = a3.map((elem) => {
    Number.isNaN(elem) == true ? (elem = elem) : (elem = Math.floor(elem));
    return elem;
  });
  return a3_res;
}
...
function t5() {
  a5_res = a5.filter((item) => {
    if (Number.isInteger(item) == true && item % 2 == 0) {
      return true;
    }
  });
  return a5_res;
...
function t6() {
  a6_res = a6.filter((item) => {
    if (Number.isInteger(item) == true && item > 14) {
      return true;
    }
  });
  return a6_res;
...
function t10() {
  let temp = [];
  a10.forEach((elem) => {
    elem > 0 ? temp.push(elem) : temp.push(0);
  });
  return (a10 = temp);
}
...
function t14() {
  if (a14.includes(a14_sym) == true) {
    return true;
  } else if (a14.includes(a14_sym.toUpperCase()) == true) {
    return true;
  } else {
    return false;
  }
