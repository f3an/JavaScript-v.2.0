...
const ajaxRequest = (Url, method, out) => {
  const xhr = new XMLHttpRequest();

  xhr.onreadystatechange = function () {
    if (this.readyState === DONE_READY_STATE && this.status === OK_HTTP_STATUS)
      displayText(out, this.responseText);
  };
  ...
  if (method === "GET") {
    ...
    xhr.send(`?auth=${params.AUTH_TOKEN}&
    action=${params.ACTION}`);
  }
};

let displayText = (out, data) => {
  out.textContent = data;
};

const createRequestUrl = (requestUrl, items) => {
  return (
    requestUrl +
    `?auth=${items.AUTH_TOKEN}&
    action=${items.ACTION}&
    name=${items.NAME}&
    num1=${items.NUM1}&
    num2=${items.NUM2}&
    year=${items.BIRTH_YEAR}&
    y=${items.YEAR}&
    d=${items.Day}&
    m=${items.Mounth}`
  );
...
function t1() {
  ...
  ajaxRequest(createRequestUrl(requestUrl, params), "GET", out);
}
...
function t10() {
  ...
  ajaxRequest(createRequestUrl(requestUrl, params), "POST", out);
}
