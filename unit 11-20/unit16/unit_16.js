...
function f5() {
  ...
  for (const item of a5) {
    item > 7 ? temp.push(item) : (temp = temp);
  }
  return temp;
}
...
function f11() {
  ...
  let str = "";
  for (const prop in a11) {
    a11[prop] > 10 ? str += `${a11[prop]} ` : str += "";
  }
  out.textContent = str;
}
...
function f15() {
  const out = document.querySelectorAll(".out-15");
  for (const div of out) {
    div.textContent = "15"
  }
}
